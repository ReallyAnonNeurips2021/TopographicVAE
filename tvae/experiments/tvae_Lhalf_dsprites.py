...
def main():
    config = {
        ...
        'mu_init': 30.0,
        'n_off_diag': 0,
        'group_kernel': (15, 15, 1),
        'n_is_samples': 10
        }

    name = 'TVAE_dSprites_L=1/2_K=1'
    ...
    for e in range(config['max_epochs']):
        ...
        total_loss, total_neg_logpx_z, total_kl, total_eq_loss, total_dis_corr, num_batches = train_epoch_dsprites(model, optimizer, 
                                                                     ...
                                                                     plot_transform_selectivity=False,
                                                                     plot_allcapoffsets=False, 
        ...
        for idx, t in enumerate(['shape', 'scale', 'orientation', 'posX', 'posY']):
            log("Epoch Avg Dist Corr: {}".format(t), total_dis_corr[idx])

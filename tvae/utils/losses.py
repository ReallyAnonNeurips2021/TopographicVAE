...
def get_allcap_offsets(allcap_offsets, alltrue_offsets, s, label, bsz, seq_len, n_caps, cap_dim):
    label = label.view(bsz, seq_len, -1)
    s = s.view(bsz, seq_len, n_caps, cap_dim).detach().cpu()
    transform_idxs = torch.argmax(torch.abs(label[:, 0] - label[:, 1]), dim=1)
    ...
    for b in range(bsz):
        ...
        for i in t0:
            pcc = periodic_corr(s[b, 0], s[b, i])
            max_pcc_locs = np.argmax(pcc, axis=1)
            cos.append(max_pcc_locs)
            tos.append(label[b, 0, transform_idx].item())
            diffs.append(abs(tos[-1]-cos[-1]))
        offset_idx = np.argmin(np.vstack(diffs), axis=0)

        for cap_idx in range(len(offset_idx)):
            allcap_offsets[transform_idx][cap_idx].append(cos[offset_idx[cap_idx]][cap_idx])
            alltrue_offsets[transform_idx][cap_idx].append(tos[offset_idx[cap_idx]])

    return allcap_offsets, alltrue_offsets
